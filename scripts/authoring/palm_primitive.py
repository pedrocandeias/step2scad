...
# --- 1. base plate: the flat palmar floor — REAL outline + basket-weave vents
# measured outline (ref section at z5.3, shapely-simplified to 40 pts) and the
# perforation grid (basket-weave: 3.21x2.13 holes, pitch 4.9, alternating
# orientation by row — matches the exact vent faces).
...
add.append({"prim": "extrude", "axis": "z", "name": "base_plate",
            ...
            "z0": Z_BOT, "z1": Z_DECK,
            "source": "flat palmar floor: measured outline (ref z5.3, 40 pts) "
                      "between exact planes #829 (z4.62) / #828 (z6.62)"})
# basket-weave vent grid, clipped to points inside the floor outline
from matplotlib.path import Path as _P
_poly = _P(np.array(floor_outline))
...
for col in range(-6, 4):
    for row in range(-5, 5):
        ...
        if not _poly.contains_point((cx, cy)):
            continue
        lx, ly = (HOLE_L, HOLE_S) if row % 2 == 0 else (HOLE_S, HOLE_L)
        cut.append(box(f"vent_{col+6}_{row+5}", (cx - lx/2, cy - ly/2, Z_BOT - 0.5),
                       (lx, ly, Z_DECK - Z_BOT + 1),
                       "basket-weave vent slot (measured 3.21x2.13, pitch 4.9)"))
        nv += 1
print(f"  grelha de respiradouros: {nv} furos dentro do contorno")
